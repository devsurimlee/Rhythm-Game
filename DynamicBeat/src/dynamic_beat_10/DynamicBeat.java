...
public class DynamicBeat extends JFrame {
	...
	public void screenDraw(Graphics2D g) { //그래픽스2D 매개변수로 변환
		...
		if(isGameScreen) {
			...
			g.drawImage(noteBasicImage, 0	, 580, null);
			...
			g.drawImage(noteBasicImage, 744, 325, null);  //뒷 부분에 그려지는 이미지일수록, 실제로는 앞쪽에 보이는 이미지이다.
			...
			g.drawString("Joakim Karud - Mighty Love", 20, 702);  //글씨 출력
